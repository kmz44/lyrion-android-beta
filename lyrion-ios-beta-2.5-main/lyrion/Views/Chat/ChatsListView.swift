...
struct ChatsListView: View {
    ...
    @State var showEditChat = false
    @State var threadToEdit: Thread?
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                List(selection: $selection) {
                    ...
                    ForEach(filteredThreads, id: \.id) { thread in
                        HStack {
                            VStack(alignment: .leading) {
                                ZStack {
                                    if let firstMessage = thread.sortedMessages.first {
                                        ...
                                        Text(NSLocalizedString("chatlist.untitled", comment: ""))
                                    }
                                }
                        ...
                        #elseif os(macOS)
                            .swipeActions {
                                Button(NSLocalizedString("chatlist.delete", comment: "")) {
                                    deleteThread(thread)
                                }
                                .tint(.red)
                            }
                            .contextMenu {
                                Button {
                                    ...
                                    Text(NSLocalizedString("chatlist.delete", comment: ""))
                                }
                            }
                        #endif
                            .tag(thread)
                ...
                if filteredThreads.count == 0 {
                    ContentUnavailableView {
                        Label(NSLocalizedString(threads.count == 0 ? "chatlist.no_chats" : "chatlist.no_results", comment: ""), systemImage: "message")
                    }
                }
            }
            .navigationTitle(NSLocalizedString("chats.title", comment: ""))
                ...
                .searchable(text: $search, prompt: NSLocalizedString("chatlist.search", comment: ""))
            #elseif os(macOS)
                .searchable(text: $search, placement: .sidebar, prompt: NSLocalizedString("chatlist.search", comment: ""))
            #endif
                .toolbar {
                    #if os(iOS) || os(visionOS)
                    ...
                    ToolbarItem(placement: .primaryAction) {
                        Button(action: {
                            ...
                            Label(NSLocalizedString("chatlist.new", comment: ""), systemImage: "plus")
                        }
                        .keyboardShortcut("N", modifiers: [.command])
                    }
                    #endif
