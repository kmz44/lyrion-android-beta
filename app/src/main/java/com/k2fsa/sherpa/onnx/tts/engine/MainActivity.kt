...
class MainActivity : ComponentActivity() {
    ...
    // Estado para controlar la pantalla de carga y evitar pantalla en blanco
    private val isTtsLoading = mutableStateOf(false)
    private val ttsInitError = mutableStateOf<String?>(null)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (!preferenceHelper.getCurrentLanguage().equals("")) {
            // Mostrar UI inmediatamente y lanzar la inicialización en background
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                TtsEngine.createTts(this@MainActivity, language)
                launch(Dispatchers.Main) {
                    if (TtsEngine.tts == null) {
                        ttsInitError.value = getString(R.string.error_loading_model)
                        val intent = Intent(this@MainActivity, ManageLanguagesActivity::class.java)
                    ...
                    if (GithubStar.shouldShowStarDialog(this@MainActivity)) {
                        GithubStar.starDialog(
                            this@MainActivity,
    ...
    private fun setupDisplay(
        ...
        setContent {
            Surface(
                ...
                Scaffold(
                    topBar = { TopAppBar(title = { Text("Lyrion • TTS") }) },
                    floatingActionButton = {}) {
                    Box(modifier = Modifier.padding(it)) {
                        if (isTtsLoading.value || ttsInitError.value != null) {
                ...
                item { Text(text = ttsInitError.value ?: getString(R.string.loading_tts_model)) }
                                item { Spacer(modifier = Modifier.height(12.dp)) }
                                item {
                                    androidx.compose.material3.LinearProgressIndicator(
                                        modifier = Modifier.fillMaxWidth(),
                                        color = colorResource(R.color.orabel_primary)
                                    )
                                }
                            }
                            return@Box
                            ...
                            item {
                                Slider(
                                    ...
                                    colors = SliderDefaults.colors(
                                        thumbColor = colorResource(R.color.orabel_primary),
                                        activeTrackColor = colorResource(R.color.orabel_primary)
                                    )
                                )
                            }
                                ...
                                Row(
                                    ...
                                    Checkbox(
                                        ...
                                        colors = CheckboxDefaults.colors(
                                            checkedColor = colorResource(R.color.orabel_primary)
                                        )
                                    )
                                    Text(
                    ...
                    Button(
                        ...
                        containerColor = colorResource(R.color.orabel_primary),
                        contentColor = colorResource(R.color.orabel_text_on_primary)
                                        ),
                                        onClick = {
                                            val intent = Intent(
                                                applicationContext,
                                ...
                                Row {
                                    Button(
                                        ...
                                        colors = ButtonDefaults.buttonColors(
                                            ...
                                            contentColor = colorResource(R.color.white_sherpa)
                                        ),
                                        onClick = {
                                            if (TtsEngine.tts == null) {
                                                Toast.makeText(
    ...
    private fun initAudioTrack() {
        ...
        if (tts == null) {
            Log.e(TAG, "initAudioTrack() called but TtsEngine.tts is null")
            Toast.makeText(this, R.string.error_loading_model, Toast.LENGTH_SHORT).show()
            return
        }
    ...
    // Use a larger buffer to reduce underruns during generation
    val bufLength = (minBuf * 4).coerceAtLeast(minBuf)
    Log.i(TAG, "sampleRate: $sampleRate, minBuf: $minBuf, usedBuf: $bufLength")
