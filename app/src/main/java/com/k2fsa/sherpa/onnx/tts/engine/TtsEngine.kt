...
object TtsEngine {
    ...
    fun createTts(context: Context, language: String) {
        if ((tts == null || lang != language) && !initializing) {
            initializing = true
            try {
    ...
    private fun initTts(context: Context, language: String) {
        ...
        var newDataDir = ""
        if (dataDir != null && assetDirExists(context, dataDir!!)) {
            newDataDir = copyDataDir(context, dataDir!!)
        } else {
            // Reintentar buscando en assets adicionales (por si se incluyeron desde ttsEngine-master)
            try {
                val alt = "espeak-ng-data"
                if (assetDirExists(context, alt)) {
                    newDataDir = copyDataDir(context, alt)
                } else {
                    Log.w(TAG, "espeak-ng-data no está incluido en assets; continuando sin dataDir")
                    newDataDir = ""
                }
            } catch (e: Throwable) {
                Log.w(TAG, "Fallo revisando assets espeak-ng-data: ${e.message}")
                newDataDir = ""
            }
        }
    ...
    val config = getOfflineTtsConfig(
        ...
        try {
            ...
            Log.i(TAG, "OfflineTts inicializado en ${elapsed}ms")
        } catch (e: Throwable) {
            Log.e(TAG, "Fallo inicializando OfflineTts", e)
            tts = null
        }
    }

    private fun assetDirExists(context: Context, path: String): Boolean {
        return try {
